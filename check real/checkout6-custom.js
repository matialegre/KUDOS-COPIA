...
// =====================================================
// FORZAR LOGIN ANTES DEL CHECKOUT
// Si el usuario no está autenticado, redirigir a la página de login
// =====================================================
(function forceLoginBeforeCheckout() {
  // Solo ejecutar en páginas de checkout (no en /cart)
  const currentPath = window.location.pathname;
  if (!currentPath.includes('/checkout') || currentPath.includes('/cart')) {
    return;
  }

  // Verificar si el usuario está autenticado usando la API de VTEX
    ...
    fetch('/api/vtexid/pub/authenticated/user')
      ...
      .then(function(user) {
        if (!user || !user.userId) {
          ...
          // Redirigir a la página de login con returnUrl al checkout
